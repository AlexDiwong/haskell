...
singleWildcardMatch (wc:ps) (x:xs) = if (ps == [] ||(head ps) == (head xs))  then mmap ([x] ++) (match wc ps xs) else Nothing
longerWildcardMatch (wc:ps) (x:xs) = if (ps == [] || (head ps) /= (head xs)) then mmap ([x] ++) (match wc (wc:ps) xs) else Nothing
